...
const openMicToggle = document.getElementById("openmic-toggle");
const openMicStatus = document.getElementById("openmic-status");
...
async function startMicSensing(externalStream = null) {
  try {
    const stream =
      externalStream || (await navigator.mediaDevices.getUserMedia({ audio: true, video: false }));
...
function updateOpenMicStatus(text, active = false) {
  openMicStatus.textContent = text;
  openMicStatus.classList.toggle("active", active);
}
...
function enqueueVoicePrompt(text) {
...
async function processVoiceQueue() {
  if (isSending || voiceQueue.length === 0) return;
  const next = voiceQueue.shift();
  await sendPrompt(next, { fromMic: true });
  processVoiceQueue();
}

async function sendPrompt(textOverride = null, options = {}) {
  if (isSending && textOverride) {
    voiceQueue.push(textOverride);
    return;
  }
...
async function startRecording() {
  if (openMicRecorder) {
    addBubble("System", "Stop open mic before manual recording.");
    return;
  }
...
async function sendAudioForTranscription(blob) {
  const text = await transcribeBlob(blob, { notify: true });
  if (text) {
...
async function transcribeBlob(blob, { notify = false } = {}) {
  const form = new FormData();
  form.append("file", blob, "clip.webm");
  if (notify) addBubble("System", "Transcribing your clip…");
  ...
  try {
    ...
    if (notify) addBubble("System", "Transcription ready. Edit or hit Send.");
    return text;
  } catch (err) {
    console.error(err);
    if (notify) addBubble("System", "Could not transcribe audio. Check server logs.");
    return "";
  }
}
...
async function processNextChunk() {
  ...
  if (text) {
    updateOpenMicStatus("Heard you. Sending…", true);
...
async function startOpenMic() {
  try {
    openMicStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
    openMicRecorder = new MediaRecorder(openMicStream);
    await startMicSensing(openMicStream);
    openMicRecorder.ondataavailable = (e) => {
      if (e.data.size > 0) enqueueChunk(e.data);
    };
    openMicRecorder.start(3500);
    openMicToggle.checked = true;
    recordBtn.disabled = true;
    updateOpenMicStatus("Listening…", true);
    addBubble("System", "Open mic is live. Speak naturally and I will auto-reply.");
  } catch (err) {
    console.error("Open mic failed", err);
    addBubble("System", "Open mic could not start. Check microphone permissions.");
    openMicToggle.checked = false;
    updateOpenMicStatus("Open mic idle", false);
  }
}

function stopOpenMic() {
  if (openMicRecorder && openMicRecorder.state !== "inactive") {
    openMicRecorder.stop();
  }
  if (openMicStream) {
    openMicStream.getTracks().forEach((t) => t.stop());
  }
  chunkQueue.length = 0;
  processingChunk = false;
  openMicRecorder = null;
  openMicStream = null;
...
openMicToggle.addEventListener("change", (e) => {
  if (e.target.checked) {
    startOpenMic();
  } else {
    stopOpenMic();
...
addBubble(
  "Vibe",
  "Hey there! I can chat, listen, and speak back. Toggle open mic to stay hands-free or tap the mic to record."
);
animateCloud();
